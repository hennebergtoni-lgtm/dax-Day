from datetime import datetime, timezone

from daxlab.runtime.mt5_readonly import Mt5Bar
from daxlab.runtime.shadow_soak import run_shadow_soak


def test_resume_with_no_new_bars_preserves_checkpoint_hash() -> None:
    bar = Mt5Bar(
        open_time=datetime(2026, 9, 9, 8, 0, tzinfo=timezone.utc),
        open=100.0,
        high=101.0,
        low=99.0,
        close=100.5,
    )
    first = run_shadow_soak((bar,))
    resumed = run_shadow_soak((), checkpoint=first.checkpoint)
    assert resumed.checkpoint.payload_sha256 == first.checkpoint.payload_sha256
    assert resumed.processed == first.processed
