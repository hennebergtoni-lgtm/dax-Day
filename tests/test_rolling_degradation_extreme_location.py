...
def test_extreme_locations_match_ordered_rolling_points():
    ...
    result = build_rolling_degradation_extreme_location_report(series)

    assert result.forward_r_per_trade.minimum_value == pytest.approx(-1.0)
    assert result.forward_r_per_trade.minimum_point_index == 1
    assert result.forward_r_per_trade.minimum_end_window_id == "W2"
    assert result.forward_r_per_trade.maximum_value == pytest.approx(1.0)
    assert result.forward_r_per_trade.maximum_point_index == 0
    assert result.forward_r_per_trade.maximum_end_window_id == "W1"
...
def test_missing_r_per_trade_is_excluded_from_extremes():
    series = build_rolling_backtest_forward_degradation_series(
        v11_2_active_reference(),
        (
    ...
    assert result.forward_r_per_trade.minimum_point_index == 1
    assert result.forward_r_per_trade.maximum_point_index == 1
    assert result.forward_r_per_trade.minimum_end_window_id == "W2"
    assert result.forward_r_per_trade.maximum_end_window_id == "W2"


def test_equal_extremes_choose_earliest_point_deterministically():
    series = build_rolling_backtest_forward_degradation_series(
        v11_2_active_reference(),
        (shadow_report("W1", (1.0,)), shadow_report("W2", (1.0,))),
        rolling_width=1,
    )
    result = build_rolling_degradation_extreme_location_report(series)
    assert result.forward_r_per_trade.minimum_point_index == 0
    assert result.forward_r_per_trade.maximum_point_index == 0
...
def test_hash_is_deterministic():
    ...
    first = build_rolling_degradation_extreme_location_report(series)
    second = build_rolling_degradation_extreme_location_report(series)
...
def test_unsafe_or_non_monitoring_series_fails_closed():
    ...
    with pytest.raises(ValueError, match="NO_ORDER"):
        build_rolling_degradation_extreme_location_report(unsafe)
    ...
    with pytest.raises(ValueError, match="monitoring-only governance"):
        build_rolling_degradation_extreme_location_report(non_monitoring)
