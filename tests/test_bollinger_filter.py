...
def test_completed_htf_state_is_timestamped_when_bar_is_known():
    bars = pd.DataFrame(
        {
            "datetime": pd.date_range("2026-01-01 09:00", periods=12, freq="5min"),
            "close": list(range(12)),
        }
    )
...
def test_filter_is_modular_and_can_be_disabled():
    trades = pd.DataFrame(
        {
            "bb_position": [0.8, 0.6, 0.9],
            "bb_bandwidth": [0.02, 0.03, 0.01],
            "prev_range_atr": [1.0, 1.0, 2.0],
        }
    )
    ...
    mask = filter_trades(
        trades,
        config=cfg,
        bandwidth_threshold=0.015,
        prior_range_atr_cap=1.5,
    )
