...
SCHEMA = "DAX_IG_CAND001_REAL_HOST_SHADOW_E2E_V1"
STATE_DIR = Path(".runtime/ig_cand001_shadow_e2e")
...
def validate_evidence(payload: dict[str, Any], *, head: str, now: datetime,
    ...
    require(payload["schema"] == SCHEMA and payload["exact_code_head"] == head,
            "STATE_CODE_OR_SCHEMA_DRIFT")
    ...
    rows = payload["input_window"]
    candles = runtime_candles(rows, utc(probe["observed_at_utc"]))
    latest = candles[-1].close_time
    require(rows[-1] == probe["latest_closed_m5"] and len(rows) == probe["closed_m5_count"],
            "DATA_PROJECTION_MISMATCH")
    require(probe["raw_m5_count"] == 40
            and probe["not_closed_m5_count"] == 40 - len(rows), "DATA_INCOMPLETE_RESPONSE")
    ...
    require(started <= requested <= response <= processed <= exported <= now,
            "CLOCK_REVERSED")
    require(latest <= requested and 0 <= (exported - latest).total_seconds() <= 600,
            "DATA_STALE_AT_EXPORT")
    if current:
        require(0 <= (now - latest).total_seconds() <= 600, "TELEMETRY_STALE")
...
def process_live_window(probe: dict[str, Any], rows: list[dict[str, Any]], *, head: str,
    ...
    require(received <= observed_at, "CLOCK_REVERSED")
    candles = runtime_candles(rows, received)
    require(0 <= (observed_at - candles[-1].close_time).total_seconds() <= 600,
    ...
    if prior is not None:
        ...
        require(bool(pending), "STATE_NO_NEW_CLOSED_M5")
    ...
    payload = {
        ...
        "ig_probe": probe, "input_window": rows, "decisions": decisions,
...
def summary(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "m5_freshness_state": payload["ig_probe"]["m5_freshness_state"],
