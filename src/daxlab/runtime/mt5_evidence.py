...
def build_host_evidence(
    *,
    ...
    identity_source = (
        f"MT5_HOST_EVIDENCE_V1|{kind}|{observed_at.isoformat()}|{digest}"
    )
    evidence_id = sha256(identity_source.encode()).hexdigest()
    return Mt5HostEvidence(
        "MT5_HOST_EVIDENCE_V1",
        evidence_id,
        digest,
...
def read_host_evidence(path: Path) -> Mt5HostEvidence:
    ...
    if set(raw) != expected:
        raise ValueError("invalid MT5 evidence fields")
    observed = datetime.fromisoformat(raw["observed_at"])
    rebuilt = build_host_evidence(
        observed_at=observed,
        kind=raw["kind"],
        payload=raw["payload"],
    )
    if (
        rebuilt.schema_version != raw["schema_version"]
        or rebuilt.evidence_id != raw["evidence_id"]
        or rebuilt.payload_sha256 != raw["payload_sha256"]
    ):
...
def detect_evidence_conflict(
    existing: Mt5HostEvidence,
    incoming: Mt5HostEvidence,
) -> str:
    if (
        existing.evidence_id == incoming.evidence_id
    ...
    if existing.evidence_id == incoming.evidence_id:
        return "CONFLICT_SAME_ID_DIFFERENT_PAYLOAD"
    return "DISTINCT"
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            ...
            if lowered in _FORBIDDEN or any(
                term in lowered for term in forbidden_fragments
            ):
                raise ValueError(
                    f"forbidden credential/account field at {path}.{key_text}"
                )
