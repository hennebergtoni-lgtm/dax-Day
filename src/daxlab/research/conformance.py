...
This module compares a frozen research-side expected-decision artifact with a
canonical deterministic product replay. Conformance proves semantic identity for
one declared fixture; it does not claim profitability or authorize execution.
"""
...
class ResearchExpectedDecisionsV1:
    ...
    def build(
        ...
        *,
        ...
        decision_fp = _fingerprint(expected_decision_ids)
        payload = _research_payload(
            ...
            expected_decision_ids_fingerprint=decision_fp,
    ...
    def __post_init__(self) -> None:
        ...
        _require_token(self.strategy_id, "strategy_id")
        _require_token(self.strategy_version, "strategy_version")
...
class ResearchProductConformanceV1:
    ...
    def __post_init__(self) -> None:
        ...
        for name, value in (
            ...
            _require_sha256(value, name)
        expected = _fingerprint(_conformance_payload(self))
        if self.conformance_fingerprint != expected:
...
def verify_research_product_conformance(
    *,
    ...
    _match(
        research.strategy_fingerprint,
        product.strategy_fingerprint,
    ...
    _match(
        ...
        product.strategy_fingerprint,
        "replay strategy fingerprint",
    )
    _match(
        manifest.input_fingerprint,
        research.replay_input_fingerprint,
    ...
    if (
        promotion.order_execution_authorized
        or promotion.paper_authorized
        or promotion.live_authorized
    ):
    ...
    provisional = ResearchProductConformanceV1(
        **values,
        conformance_fingerprint="0" * 64,
    )
    return ResearchProductConformanceV1(
        **values,
        conformance_fingerprint=_fingerprint(_conformance_payload(provisional)),
    )
...
def _conformance_payload(artifact: ResearchProductConformanceV1) -> dict[str, object]:
    return {
        ...
        "execution_capability": artifact.execution_capability,
        "order_execution_authorized": artifact.order_execution_authorized,
        "paper_authorized": artifact.paper_authorized,
        "live_authorized": artifact.live_authorized,
    }
