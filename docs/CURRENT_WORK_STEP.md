...
- Last completed whole-number step: **2190**
- Last interrupted whole-number step: **2185**
- Active whole-number step: **2191**
- Next step after successful completion: **2192**
- Active Step 2191 scope: **Implement only the smallest software contract needed to separate DEMO evidence-acquisition authorization from final PAPER authorization. Authorization-contract work only: explicit demo account/server/symbol/scope/expiry binding and fail-closed validation. Do not add venue submission, MT5 order APIs, broker orders, PAPER/LIVE authorization, fabricated broker evidence, risk auto-promotion, CAND-001/V11.2/cost changes, or duplicate lifecycle/reconciliation/telemetry/state owners.**
...
| 2191 | Demo-evidence authorization contract. | **ACTIVE.** Implement only a distinct fail-closed DEMO evidence-acquisition authorization contract; no venue transport, no MT5 order API, no broker order, no PAPER/LIVE authorization. |
