...
- Last completed whole-number step: **2194**
- Last interrupted whole-number step: **2185**
- Active whole-number step: **2195**
- Next step after successful completion: **2196**
- Active Step 2195 scope: **Implement only the smallest restart-safe DEMO transport-attempt reservation selected by Step 2194. REUSE the existing strict Windows MT5 bundle validator, expose the already-validated redacted DEMO account context from that same bundle, and bind it with immutable Step-2187 PREPARED evidence plus Step-2191 DEMO authorization into one deterministic tamper-evident `TRANSPORT_ATTEMPT_RESERVED` local state persisted through the existing `StateStorePort` attempt key before any future venue call. Exact replay must be idempotent; cross-wired PREPARED/auth/account/bundle/attempt ordinal must fail closed; a reserved state means reconcile/query first after restart and never blind resubmit. No venue adapter, no `mt5.order_send`, no broker order, no DEMO/PAPER/LIVE authorization, no automatic slot release/retry, no risk auto-promotion, no CAND-001/V11.2/cost changes and no duplicate lifecycle/reconciliation/telemetry/storage stack.**
...
| 2195 | DEMO transport-attempt reservation checkpoint. | **ACTIVE.** Implement only the Step-2194 local restart-safe reservation and minimal Windows-bundle account-context exposure; no venue transport/order capability. |
