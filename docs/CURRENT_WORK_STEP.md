...
- Last completed whole-number step: **2195**
- Last interrupted whole-number step: **2185**
- Active whole-number step: **2196**
- Next step after successful completion: **2197**
- Active Step 2196 scope: **REUSE the existing reservation codec and StateStorePort for a fail-closed read-only restart loader pinned to the original reservation fingerprint. Project local RESERVED / venue UNKNOWN / QUERY_RECONCILE_REQUIRED operator evidence without new storage, freshness, lifecycle transitions, authorization or slot release.**
...
| 2196 | Reserved-attempt read-only restart ownership. | **ACTIVE.** Pin load to original evidence; venue state remains UNKNOWN and query/reconcile is mandatory. |
