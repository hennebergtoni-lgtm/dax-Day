...
| Operator risk / exposure profiles | `docs/OPERATOR_RISK_PROFILE_CONTROL_V1.md` | Planned bounded profile request layer; capital allocation is not max-loss-at-stop; broker-aware sizing is still not implemented | Current normalized simulation sizing and broker-economics readiness remain separate |
| Broker economics readiness | `docs/BROKER_ECONOMICS_READINESS_V1.md`, `src/daxlab/runtime/broker_economics_readiness.py`, `tests/test_candidate_broker_economics_readiness.py`, `scripts/mt5_windows_probe.py` | Read-only venue metadata gate for future sizing research; real DE40 economics are `WAITING_EXTERNAL`; `READY_FOR_SIZING_RESEARCH` is not PAPER authorization | Current `candidate_sizing.py` remains normalized simulation-only sizing |
...
| Web/operator surface | `docs/WEB_INTERFACE_CONTRACT_V2.md`, `src/daxlab/runtime/operator_snapshot.py`, `docs/CAND001_OPERATOR_TELEMETRY_V1.md`, `web/status.json` | V2 is current: static dashboard uses `DAXLAB_WEB_STATIC_STATUS_V2`; fresh runtime source is separate Candidate telemetry and browser endpoint remains a later safe-backend task | `WEB_INTERFACE_CONTRACT_V1.md` and static V1 runtime/pre-host assertions are historical/superseded |
