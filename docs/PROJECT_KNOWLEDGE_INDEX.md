...
| NextGen canonical domain / ports | `src/daxlab/domain/market.py`, `src/daxlab/domain/execution.py`, `src/daxlab/domain/ports.py`, `tests/test_nextgen_domain_foundation.py` | Step 2139 completed broker/storage-neutral `InstrumentId`, UTC market contracts, deterministic execution intent and protocol-only ports; architecture tests forbid MT5/legacy/CAND-001/V11.2 coupling | Existing runtime contracts remain compatibility surfaces until adapted deliberately |
| NextGen canonical data catalog | `src/daxlab/data/catalog/manifest.py`, `src/daxlab/data/catalog/parquet.py`, `tests/test_nextgen_parquet_catalog.py` | Step 2140 completed immutable Parquet/Arrow Catalog V1 with content fingerprint + physical SHA256, UTC/instrument/data-quality roundtrip and fail-closed ordering/tamper rules; legacy 1,673-file CSV layout is not the storage contract | `src/daxlab/data/legacy_dataset.py` and Drive `m5_daily` are historical import/evidence sources only |
| NextGen strategy plugin boundary | `src/daxlab/domain/strategy.py`, `src/daxlab/strategies/contracts.py`, `tests/test_nextgen_strategy_plugin_contract.py` | Step 2141 COMPLETED: generic deterministic `StrategyDecision`/`TradePlan` and `StrategyPlugin`/`StrategyTransition`; strategy output deliberately excludes quantity/risk/broker/order authority | CAND-001 behavior is connected separately through the compatibility adapter |
| NextGen CAND-001 strategy adapter | `src/daxlab/strategies/cand001/adapter.py`, `tests/test_nextgen_cand001_strategy_adapter.py` | Step 2142 COMPLETED: existing CAND-001 pipeline remains behavior owner and is exposed through the generic Strategy Plugin boundary without adding execution capability | Frozen CAND-001 runtime/pipeline remains compatibility behavior source |
| NextGen deterministic replay engine | `src/daxlab/engine/replay.py`, `tests/test_nextgen_replay_engine.py` | Step 2143 COMPLETED: generic broker/storage-neutral fail-closed Product Replay Engine over canonical Candle + Strategy Plugin semantics; execution capability remains `NONE` | Legacy historical/CAND-001 replay remains evidence/compatibility provenance |
| NextGen research experiment / promotion | `src/daxlab/research/promotion.py`, `tests/test_nextgen_research_promotion.py`, `docs/STEP_2144_IMPLEMENTATION_NOTE.md` | Step 2144 COMPLETED: deterministic experiment/product/promotion identities bind strategy/config, dataset, split/WF, costs/fills, robustness evidence and source provenance; promotion never authorizes orders/PAPER/LIVE | Existing `ExperimentManifest`/registry/research-gate sources remain upstream provenance/governance inputs |
| NextGen research→product conformance | `src/daxlab/research/conformance.py`, `tests/test_nextgen_research_product_conformance.py` | Step 2145 COMPLETED: frozen canonical fixtures bind experiment/product/dataset/replay-input and ordered strategy-decision identities; provenance/input/semantic drift fails closed | Research metrics remain research evidence, never execution authorization |
| NextGen MT5 market-data adapter | `src/daxlab/adapters/mt5_market_data.py`, `tests/test_nextgen_mt5_market_data_adapter.py`, `docs/MT5_ADAPTER_CONTRACT_V1.md` | Step 2146 COMPLETED: read-only validated MT5/ClosedM5 feed is translated into canonical UTC Candles behind `CandleSourcePort`; no MT5 SDK or order dependency in the adapter | Real host clock/GREEN verification remains the external lane originating in Step 2122 |
| NextGen state-store adapter | `src/daxlab/adapters/file_state_store.py`, `tests/test_nextgen_file_state_store.py`, `src/daxlab/domain/ports.py` | Step 2147 COMPLETED: atomic opaque-byte file-backed `StateStorePort` with fsync/replace, exact roundtrip and fail-safe replacement behavior | Legacy/specialized recovery owners remain separate compatibility/forensic surfaces |
| NextGen operator read model | `src/daxlab/operator/read_model.py`, `src/daxlab/adapters/cand001_operator.py`, `tests/test_nextgen_operator_read_model.py` | Step 2148 COMPLETED: generic read-only product operator view plus CAND-001 compatibility adapter; no alert/control/order authority | Existing Candidate telemetry/operator payload remains compatibility/evidence truth for that product line |
| NextGen deterministic checkpoint | `src/daxlab/state/replay_checkpoint.py`, `tests/test_nextgen_replay_checkpoint.py` | Step 2149 COMPLETED: `ProductCheckpointV1` binds run/strategy/config/source/input/event/decision/state-codec identity with canonical tamper-checked serialization behind `StateStorePort` | No broker/live resume behavior is implied by the artifact alone |
| NextGen replay restart/resume | `src/daxlab/engine/resume.py`, `tests/test_nextgen_replay_resume.py` | Step 2150 COMPLETED: generic `StrategyStateCodec` interruption/resume path reproduces uninterrupted ordered decisions and final strategy state exactly across multiple split points | This is deterministic replay/product-state evidence, not broker/PAPER authorization |
| NextGen CAND-001 state codec / resume parity | `src/daxlab/strategies/cand001/state_codec.py`, `tests/test_nextgen_cand001_resume.py` | Step 2151 COMPLETED: strict canonical JSON codec for causal CAND-001 pipeline state, explicit UTC/canonical form/finite-value validation and real Candidate resume parity; no pickle or execution capability | Existing CAND-001 pipeline remains sole strategy-behavior owner |
