...
| NextGen research experiment / promotion | `docs/CURRENT_WORK_STEP.md`, `src/daxlab/contracts.py`, `docs/RESEARCH_GATES.md`, `docs/RESEARCH_MODULE_CATALOG.md`, current research registry owner(s) | **Step 2144 IN PROGRESS.** First action is audit/reuse of existing ExperimentManifest/registry provenance. Target artifact must bind strategy/product, dataset, split, costs/fills, robustness and source provenance while explicitly carrying `order_execution_authorized=false`. No optimizer/broker/auto-promotion is authorized. | Existing research manifests/registry are candidates for KEEP/ADAPT; do not create a duplicate registry before audit |
