...
| NextGen strategy plugin boundary | `src/daxlab/domain/strategy.py`, `src/daxlab/strategies/contracts.py`, `docs/CURRENT_WORK_STEP.md` | **Step 2141 IN PROGRESS.** Generic `StrategyDecision`/`TradePlan` and `StrategyPlugin`/`StrategyTransition` exist; strategy output deliberately excludes quantity/risk/broker/order authority. Dedicated Strategy-Conformance test is still required before step closure. | CAND-001 strategy semantics are not yet migrated into this boundary |
