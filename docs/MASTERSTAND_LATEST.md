...
**Current2233 overlay:** 60s-policy real-host fresh-start succeeded, but subsequent resume again correctly blocked changed overlap (nominal12:25 close25405.6->25406.6, volume175->192). Raw12:45 observed approx.12:45:47 later changed volume29->125, as supplied by user. Finality and timestamp semantics remain UNKNOWN.2233 is UNVERIFIED / RAW HOST EVIDENCE WAITING_EXTERNAL. First use the one-shot credential-free RAW diagnostic and three-snapshot/two-boundary runbook in `STEP_2233_IG_RAW_TIMESTAMP_TRUTH.md`; no guessed replacement grace, normalization change or overlap relaxation. Existing files and prior2231/2232 initial proof are preserved. Full PHASE2–12 mandate received. C: OTHER / UNKNOWN pending complete RAW source observations. Live Candidate fresh-start/resume blocks DATA_IG_M5_PROVIDER_CONTRACT_UNVERIFIED before input/processing/evidence; no CLI override. RAW V2 row/hash/time/leaf-diff/UNKNOWN-state diagnostics available in separate preserved namespace .runtime/ig_raw_m5_truth_2233_v2. No final Candidate contract, migration or namespace chosen. NONE/false; no broker mutations, strategy/risk/cost/V11.2 change, merge or Acceptance refresh. No subsequent whole-number step is activated.

Status: **BINDING LATEST HANDOVER / REPOSITORY TRUTH FIRST**  
Updated: **2026-09-14 — Step 2233 reopened / RAW timestamp truth WAITING_EXTERNAL**
...
The ten-row diagnostic reproduces a wrong OHLC/time mapping, not the freshness exception itself. Paging is a separately reproduced request-contract defect; its causal role on the failing host still needs real metadata/rerun evidence. Official IG REST documentation does not formally specify start versus end labels. See **`docs/IG_DEMO_M5_TIMESTAMP_HANDOFF.md`** for exact evidence, tests/CI and the single next read-only Windows command. Validated implementation head `a34205b66ddb68d401dc88ceb38a2146cc745001`: DAX #668 / run 34830283471 and Research #1452 / run 34830283580 GREEN; 70 IG, 1069 relevant and 2689 full local tests passed (6 local pwsh skips); Research CI 2695 passed; Ruff/syntax/eight offline gates passed. The subsequent handoff-only commit preserves the code/test tree and needs its own fresh required checks. Re-pin the current publication head and required CI rather than deploy the old start SHA.
...
## 2026-09-14 — 2232 real Windows closeout / active2233

This overlay supersedes prior pending2232 pointers. The user supplies binding actual Windows E2E verification at head7b715f958cd6c2a2bd4055e0e5914aea5519ba47: live/operator exit0, IG raw40/closed40 -> CAND-001 Decision/SHADOW -> telemetry/operator, GREEN, FRESH, NO_TRADE/NONE/OR_INCOMPLETE/NO_SIGNAL, NONE/false, fingerprint present, no broker side effects. **2232 COMPLETED / VERIFIED** for the initial E2E scope. Original JSON bytes/hash were not delivered to this Work; no independent host run is claimed.

Subsequent resume correctly blocked STATE_CHANGED_OVERLAP after retrospective OHLC/volume revision of nominal close11:40UTC; another response observed close12:05UTC approx.3.7s old. Active **2233 PLANNED**: narrow 60s Candidate-only IG finalization grace, unchanged600s freshness/strict overlap, incompatible manifest/evidence contract and preserved2232 files/new2233 namespace. No aggressive authentication retries after supplied sporadic401. Next2234 after real fresh-start+resume+operator evidence; broaderM01/2206/2122 unchanged, protection/reconciliation UNKNOWN. No strategy/risk/cost/MT5/execution/V11.2 change or orders/merge/Acceptance refresh.


## 2026-09-14 — Step2233 implemented / new contract rerun pending

Latest overlay supersedes the planned2233 status above: **2233 IMPLEMENTED_LOCAL / REAL WINDOWS RERUN WAITING_EXTERNAL**, last completed2232, next2234. Candidate-only grace60 uses request-start observation, preventing transport/processing boundary promotion. General IG nominal closure/probe/UTC/paging unchanged; current Candidate age60..600 inclusive. V2 evidence separates nominal observations from eligible input, binds exact policy in dataset/manifest and complete existing checkpoint, and exposes actual eligible current/identity/recovery in telemetry/operator. No strategy/state engine or supervisor added.

Legacy contract blocks STATE_FINALIZATION_CONTRACT_MIGRATION_REQUIRED before auth; manifest mismatch STATE_MANIFEST_DRIFT. Existing2232 files are preserved; new2233 defaultnamespace. STATE_CHANGED_OVERLAP remains exact over processed OHLC/volume/source/timestamps, no tolerance/reset. No new eligible bar blocks STATE_NO_NEW_FINALIZED_M5;401 returns fixed IG_AUTHENTICATION_FAILED_NO_RETRY without loop/provider text.

Local102 focused and2736 full tests passed,6 local PowerShell skips; Ruff/syntax/imports passed. Final CI/offline gates must be read on the actual publication tree. Runbook docs/STEP_2233_IG_M5_FINALIZATION_HANDOFF.md requires separate actual Windows fresh-start and later new-finalized-bar resume with successful hash-bound operator reads and both source bundles.60s is policy, not proof against later provider revision. Protection/reconciliation UNKNOWN, broaderM01/2206/2122 unchanged; NONE/false, SHADOW only, no orders/controls/merge/force/Acceptance refresh or V11.2/CAND-001/risk/cost/MT5/execution changes.
